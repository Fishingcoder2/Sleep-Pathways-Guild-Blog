#!/usr/bin/env python3
"""Inventory visible links on the public Sleep Pathways Guild downloads hub.

The report includes visible button text, HTTP status, redirects, and sign-in
redirect detection so a public-looking download that actually requires an
...
def main() -> int:
    ...
    lines = [
        "Sleep Pathways Guild downloads hub inventory",
        f"Hub: {URL}",
        f"Hub status: {status}",
        "",
    ]
    ...
    for anchor in parser.anchors:
        ...
        code, destination = probe(absolute)
        blocked_by_auth = auth_redirect(absolute, destination)
